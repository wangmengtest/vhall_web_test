...
class Tool(PreviewRequest):
    ...
    def thirdLogin(self, data):
        requestUtil.openDocHook("中建-三方登录", self.dbUtil, ["rooms", "room_supply"], False)
    ...
    def loginAssistant(self, data):
        requestUtil.openDocHook("中建跳转-观众登录", self.dbUtil, ["rooms", "room_supply"], False)
        return self.post("api/auth/login-watch", data)

    def testRedisHash(self, data):
        requestUtil.openDocHook("测试redisHash类型", self.dbUtil, ["rooms", "room_supply"], False)
        return self.post("api/auth/test-redis-hash", data)

    def testRedisDel(self, data):
        requestUtil.openDocHook("测试redis删除Key和值", self.dbUtil, ["rooms", "room_supply"], False)
        return self.post("api/auth/test-redis-del", data)

    def testRedis(self, data):
        requestUtil.openDocHook("测试redis set和get", self.dbUtil, ["rooms", "room_supply"], False)
        return self.post("api/auth/test-redis", data)
