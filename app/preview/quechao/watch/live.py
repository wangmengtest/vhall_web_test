...
class Live(PreviewRequest):
    ...
    def npsReviewsScore(self, data):
        requestUtil.openDocHook("观看端-NPS-评分", self.dbUtil, ["rooms"])
        return self.post("api/live/nps-score", data)

    def npsReviewsReviews(self, data):
        requestUtil.openDocHook("观看端-NPS-评价", self.dbUtil, ["rooms"])
        return self.post("api/live/nps-reviews", data)

    def npsReviewsStatList(self, data):
        requestUtil.openDocHook("观看端-NPS-统计列表", self.dbUtil, ["rooms"])
        return self.post("console/npsreviews/stat-list", data)
